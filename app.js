...
const TOP={x:393,y:105},BOTTOM={x:207,y:205},OFF=[[-42,-42],[44,-27],[42,42],[-44,27]],ALT={2:4,3:5,4:2,5:3};
const corresponding=p=>p+4;
function point(p){return p<4?TOP:BOTTOM}
function wedge(p){return p<4?p:p-4}
function angleGeometry(angle){
  const theta=angle>90?180-angle:angle,rad=theta*Math.PI/180;
  // 오른쪽 위/아래 횡단선의 실제 기울기를 theta에 맞춘다.
  const slope=Math.tan(rad),x1=85,x2=515,mid=155,half=215;
  return {theta,line:`M${x1} ${(mid+slope*half).toFixed(2)}L${x2} ${(mid-slope*half).toFixed(2)}`};
}
function arcPath(cx,cy,p,r,theta){
  const a=[[-180,-theta],[-theta,0],[0,180-theta],[180-theta,180]][p],rad=d=>d*Math.PI/180;
  const x1=cx+r*Math.cos(rad(a[0])),y1=cy+r*Math.sin(rad(a[0])),x2=cx+r*Math.cos(rad(a[1])),y2=cy+r*Math.sin(rad(a[1]));
  return `M${x1.toFixed(1)} ${y1.toFixed(1)}A${r} ${r} 0 0 1 ${x2.toFixed(1)} ${y2.toFixed(1)}`;
}
function labelPoint(cx,cy,p,r,theta){const mids=[-(180+theta)/2,-theta/2,(180-theta)/2,(180+theta)/2],a=mids[p]*Math.PI/180;return [cx+r*Math.cos(a),cy+r*Math.sin(a)]}
function labels(){const x=shuffle(LETTERS),m={};x.forEach((v,i)=>m[i]=v);return m}
function baseSvg(line,theta,items){let s=`<svg viewBox="0 0 600 300"><style>.g{stroke:#111827;stroke-width:4;fill:none;stroke-linecap:round}.arc{stroke:#6b7280;stroke-width:2;fill:none}.t{font:700 21px system-ui,sans-serif;fill:#111827;text-anchor:middle;dominant-baseline:middle}.d{stroke:#6b7280;stroke-width:1.5;stroke-dasharray:5 5}</style><path class="g" d="M45 105H555M45 205H555M${line}"/>`;items.forEach(it=>{s+=`<path class="arc" d="${arcPath(it.p.x,it.p.y,it.w,30,theta)}"/>`;const z=labelPoint(it.p.x,it.p.y,it.w,58,theta);s+=`<text class="t" x="${z[0].toFixed(1)}" y="${z[1].toFixed(1)}">${it.text}</text>`});return s+'</svg>'}
function diagram(m){const theta=60,g=angleGeometry(theta),items=[];for(let i=0;i<4;i++)items.push({p:TOP,w:i,text:m[i]});for(let i=0;i<4;i++)items.push({p:BOTTOM,w:i,text:m[i+4]});return baseSvg(g.line,theta,items)}
function sizeDiagram(q){const g=angleGeometry(q.angle),items=[{p:q.targetPoint,w:q.targetPos,text:'a'},{p:q.givenPoint,w:q.givenPos,text:q.value+'°'}];let s=baseSvg(g.line,g.theta,items);return s.replace('</svg>','<path class="d" d="M45 80H555M45 230H555"/></svg>')}
function newQuestion(){
 const type=1+Math.floor(Math.random()*4);
 if(type===1){const m=labels(),p=+Object.keys(m).find(k=>m[k]==='a');return{type,m,answer:m[corresponding(p)],text:'각 a의 동위각은?'}}
 if(type===2){const m=labels(),p=pick([2,3,4,5]),a=+Object.keys(m).find(k=>m[k]==='a');[m[p],m[a]]=[m[a],m[p]];return{type,m,answer:m[ALT[p]],text:'각 a의 엇각은?'}}
 let target,given;if(type===3){target=Math.floor(Math.random()*8);given=corresponding(target)}else{target=pick([2,3,4,5]);given=ALT[target]}
 const value=pick(ANGLES),targetPoint=point(target),givenPoint=point(given);
 const givenAcute=given===1||given===2,valueAcute=value<90,theta=givenAcute===valueAcute?value:180-value;
