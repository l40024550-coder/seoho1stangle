...
function makeSizeQuestion(){
  ...
  const usable=choices.filter(c=>{
    const value=actualAngle(c.given,theta);
    return c.given!==target&&value!==answer&&!recentGivenValues.includes(value);
  });
  ...
  remember(recentSizeAnswers,answer,10);
  remember(recentGivenValues,givenValue,5);
