...
function start(test=false){if(!test){player.studentNo=$('studentNo').value.trim();player.name=$('studentName').value.trim();if(!player.studentNo||!player.name){$('homeMessage').textContent='학생 번호와 이름을 입력하세요.';return;}}if(wrongUnlockTimerId){clearTimeout(wrongUnlockTimerId);wrongUnlockTimerId=null;}clearInterval(timerId);state={score:0,correct:0,wrong:0,index:0,test,finished:false};locked=false;submitting=false;show('game');$('score').textContent='0';$('feedback').textContent='';let t=test?TEST_TIME:NORMAL_TIME;$('timer').textContent=t;render();timerId=setInterval(()=>{if(!state)return; t--; $('timer').textContent=t;if(t<=0)end();},1000);}
function submit(e){e.preventDefault();if(!state||state.finished||locked||submitting)return;submitting=true;const s=state,q=s.q;if(!q){submitting=false;return;}const answer=$('answerInput').value.trim().toLowerCase();const ok=answer===String(q.answer).trim().toLowerCase();if(ok){s.score++;s.correct++;s.index++;$('score').textContent=s.score;$('feedback').textContent='정답! +1점';submitting=false;render();return;}s.score-=2;s.wrong++;$('score').textContent=s.score;$('feedback').textContent='오답! -2점 · 3초 동안 입력할 수 없습니다.';locked=true;$('answerInput').disabled=true;if(wrongUnlockTimerId)clearTimeout(wrongUnlockTimerId);wrongUnlockTimerId=setTimeout(()=>{wrongUnlockTimerId=null;if(state!==s||s.finished)return;locked=false;s.index++;submitting=false;render();},3000);}
